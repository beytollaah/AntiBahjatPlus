...
def video_func(video, keep_audio, defacing_sensitivity, audio_change_coef):
    ...
    if keep_audio:
        ...
        if audio_change_coef != 0:
            ...
            voice_change_command = 'asetrate=' + str(tone_freq) + ',aresample=44100,' + 'atempo=' + str(
                1 / (1 + audio_change_coef))
            subprocess.run(
                ['ffmpeg', '-i', name + '_audio.wav', '-af', voice_change_command, name + '_altered_audio.wav'])

            # add altered audio the the new video
            subprocess.run(
                ["ffmpeg", '-i', name + '_anonymized.' + extension, '-i', name + '_altered_audio.wav', '-map', '0:v',
                 '-map', '1:a', '-c:v', 'copy', '-shortest', name + '_anonymized_with_audio.' + extension])
        else:
            # add the original audio to the new video
            subprocess.run(
                ["ffmpeg", '-i', name + '_anonymized.' + extension, '-i', name + '_audio.wav', '-map', '0:v', '-map',
                 '1:a', '-c:v', 'copy', '-shortest', name + '_anonymized_with_audio.' + extension])
...
def get_gradio_ui():
    with gr.Blocks() as ui:
        ...
        defacing_sensitivity = gr.Slider(minimum=0.01, maximum=0.99, value=0.9, interactive=True,
                                         label='Sensitivity of face detection/حساسیت تشخیص چهره')
        audio_change_coef = gr.Slider(minimum=-0.49, maximum=0.99, value=0, interactive=True,
                                      label='Audio Change Coefficient/ضریب تغییر صدا')
        btn = gr.Button("Run")
        out = gr.File()

        btn.click(fn=video_func, inputs=[video, keep_audio, defacing_sensitivity, audio_change_coef], outputs=out,
                  queue=True)
